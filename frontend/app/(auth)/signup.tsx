  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1, backgroundColor: colors.surface }}>
      <ScrollView contentContainerStyle={[styles.container, { paddingTop: insets.top + 24, paddingBottom: insets.bottom + 24 }]} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.subtitle}>Join thousands of nurses building a global career.</Text>

        <View style={{ gap: 12, marginTop: 24 }}>
          <TextInput testID="signup-name-input" placeholder="Full name" placeholderTextColor={colors.muted} value={name} onChangeText={setName} style={styles.input} />
          <TextInput testID="signup-email-input" placeholder="Email" placeholderTextColor={colors.muted} autoCapitalize="none" keyboardType="email-address" value={email} onChangeText={setEmail} style={styles.input} />
          <TextInput testID="signup-password-input" placeholder="Password (6+ chars)" placeholderTextColor={colors.muted} secureTextEntry value={password} onChangeText={setPassword} style={styles.input} />
        </View>

        {!!err && <Text style={styles.err} testID="signup-error">{err}</Text>}

        <Pressable testID="signup-submit-btn" style={[styles.primary, busy && { opacity: 0.7 }]} onPress={submit} disabled={busy}>
          <Text style={styles.primaryText}>{busy ? "Creating..." : "Create account"}</Text>
        </Pressable>

        <Link href="/(auth)/login" style={styles.linkText} testID="go-login">
